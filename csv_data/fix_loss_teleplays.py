...
from crawler.get_details import Teleplay


def parse_teleplay_page(html):
    ...
    return Teleplay(image_link=image_link, title=title, star=star, leader=leader, tags=tags, country=country, director_description=director, years=show_time, id=index, description=description, time_length=0, imdb_link='None', language=language)


df = pd.read_csv('teleplay_links.csv', header=None)
df2 = pd.read_csv('teleplays_3.csv')
print(df2.columns)
...
for index, value in df.iterrows():
    ...
    text = fetch_html(url)
    print('parsing ....', value)
    teleplay = parse_teleplay_page(text)
    ...
    data = {'id': index, 'title': teleplay.title, 'image_link': teleplay.image_link, 'country': teleplay.country, 'years': teleplay.years, 'director_description': teleplay.director_description, 'leader': teleplay.leader, 'star': teleplay.star, 'description': teleplay.description, 'tags': '/'.join(teleplay.tags), 'imdb': teleplay.imdb_link, 'language': teleplay.language, 'time_length': teleplay.time_length}
...
df2.to_csv('fix_teleplay.csv', index=False)
