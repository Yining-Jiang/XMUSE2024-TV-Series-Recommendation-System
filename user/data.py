...
# sys.path.extend(['D:\\github\\user-recommendation-system\\python-django\\teleplaySystem', ])
...
# 设置url
urls = set('https://teleplay.douban.com/top250?start={}'.format(str(i)) for i in range(0, 250, 25))

page_count = 0


# 获得url的电视剧
def get_url_teleplay():
    global page_count
    while urls:
        url = urls.pop()
        page_count += 1
        ...
        teleplay_hrefs = soup.select('div.pic > a')
        for teleplay_href in teleplay_hrefs:
            print('fetch user href', teleplay_href)
            get_info_teleplay(teleplay_href['href'])
        time.sleep(2)
...
def save_images(link, name):
    ...
    image_name = '../teleplay_images/' + name + '.png'
    print(image_name)
